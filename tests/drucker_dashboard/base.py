...
POSITIVE_MODEL_PATH = pathlib.Path(__file__).parent.joinpath('models').joinpath('positive.pkl')
NEGATIVE_MODEL_PATH = pathlib.Path(__file__).parent.joinpath('models').joinpath('negative.pkl')
...
class BaseTestCase(TestCase):
    ...
    def create_app(self):
        from drucker_dashboard.app.app import initialize_app
        app = Flask(__name__)
        initialize_app(app)
        return app
...
def stop_worker(config_path, namespaces=('development', 'beta')):
    ...
    for ns in namespaces:
        ...
        for svc in core_v1.list_namespaced_service(ns).items:
            core_v1.delete_namespaced_service(name=svc.metadata.name, namespace=ns)
        # Delete Ingresses
